...
export class SupplyChainStepsService {
  ...
  async create(createSupplyChainstepsDto: CreateSupplyChainStepsDto) {
    ...
    supplyChain.from = await this.userService.findOne(createSupplyChainstepsDto.from)
    supplyChain.to = await this.userService.findOne(createSupplyChainstepsDto.to)
  ...
  findAll() {
    return this.supplychainStepRepo.find({
        relations:['product','service','transport','rawMaterial','from','to']
      });
  }
